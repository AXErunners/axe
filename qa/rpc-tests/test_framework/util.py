...
def wait_for_bitcoind_start(process, url, i):
    '''
    Wait for axed to start. This means that RPC is accessible and fully initialized.
    Raise an exception if axed exits during initialization.
    '''
    while True:
        if process.poll() is not None:
            raise Exception('axed exited with status %i during initialization' % process.returncode)
        try:
...
def initialize_chain(test_dir, num_nodes, cachedir, extra_args=None, redirect_stderr=False):
    ...
    if create_cache:
        ...
        for i in range(MAX_NODES):
            ...
            args = [ os.getenv("BITCOIND", "axed"), "-server", "-keypool=1", "-datadir="+datadir, "-discover=0", "-mocktime="+str(GENESISTIME) ]
            if i > 0:
            ...
            logger.debug("initialize_chain: axed started, waiting for RPC to come up")
    ...
    for i in range(num_nodes):
        ...
        initialize_datadir(test_dir, i) # Overwrite port/rpcport in axe.conf
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, redirect_stderr=False, stderr=None):
    """
    Start a axed and return RPC connection to it
    """
    datadir = os.path.join(dirname, "node"+str(i))
    if binary is None:
        binary = os.getenv("BITCOIND", "axed")
...
def start_nodes(num_nodes, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, redirect_stderr=False):
    """
    Start multiple axeds, return RPC connections to them
    """
