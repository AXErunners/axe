...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "AxeNetwork") type = AxeNetwork;
    if(strType == "AxeNetworkVariable") type = AxeNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == AxeNetwork) s = "AxeNetwork";
    if(type == AxeNetworkVariable) s = "AxeNetworkVariable";
