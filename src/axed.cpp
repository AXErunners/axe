 ...
 *
 * This is the developer documentation of the reference client for an experimental new digital currency called Axe (https://www.axe.org/),
...
bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/axe.conf are parsed in qt/axe.cpp's main()
    ParseParameters(argc, argv);

    // Process help and version before taking care about datadir
    if (mapArgs.count("-?") || mapArgs.count("-h") ||  mapArgs.count("-help") || mapArgs.count("-version"))
    {
        std::string strUsage = _("Axe Core Daemon") + " " + _("version") + " " + FormatFullVersion() + "\n";
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  axed [options]                     " + _("Start Axe Core Daemon") + "\n";
    ...
    try
    {
        ...
        if (fCommandLine)
        {
            fprintf(stderr, "Error: There is no RPC client functionality in axed anymore. Use the axe-cli utility instead.\n");
            exit(EXIT_FAILURE);
        }
#ifndef WIN32
