...
#if defined(NDEBUG)
# error "Axe Core cannot be compiled without assertions."
#endif
...
public:
    ...
    bool ActivateBestChain(CValidationState &state, const CChainParams& chainparams, std::shared_ptr<const CBlock> pblock, bool fSyncQueue = true);
                                     ...
                                     const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    for (const CTxIn &txin : tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in Axe
...
static int64_t nTimeAxeSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // AXE : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if (sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        for (const auto& tx : block.vtx) {
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                return state.DoS(10, error("ConnectBlock(AXE): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                                 REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else {
        LogPrintf("ConnectBlock(AXE): spork is off, skipping transaction locking checks\n");
    }
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(AXE): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward)) {
        return state.DoS(0, error("ConnectBlock(AXE): couldn't find masternode or superblock payments"),
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(AXE): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeAxeSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - Axe specific: %.2fms [%.2fs (%.2fms/blk)]\n", MICRO * (nTime5 - nTime4), nTimeAxeSpecific * MICRO, nTimeAxeSpecific * MILLI / nBlocksTotal);
 ...
 */
bool CChainState::ActivateBestChain(CValidationState &state, const CChainParams& chainparams, std::shared_ptr<const CBlock> pblock, bool fSyncQueue) {
...
bool ActivateBestChain(CValidationState &state, const CChainParams& chainparams, std::shared_ptr<const CBlock> pblock, bool fSyncQueue) {
    return g_chainstate.ActivateBestChain(state, chainparams, std::move(pblock), fSyncQueue);
}
...
bool CChainState::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs, const CChainParams& params)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, false /*fJustCheck*/, false /*fScriptChecks*/)) {
        return error("RollforwardBlock(AXE): ProcessSpecialTxsInBlock for block %s failed with %s",
            pindex->GetBlockHash().ToString(), FormatStateMessage(state));
    }
