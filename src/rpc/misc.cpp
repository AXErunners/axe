...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("debug", "axe")
            + HelpExampleRpc("debug", "axe+net")
        );
    ...
    if (request.params.size() == 1) {
        ...
        if (strCommand == "show") {
            ...
            for (const auto& sporkDefM : sporkDefsM) {
                ret.pushKV(sporkDefM.name, sporkManager.GetSporkValue(sporkDefM.sporkId));
            };
            for (const auto& sporkDefT : sporkDefsT) {
                ret.pushKV(sporkDefT.name, sporkManager.GetSporkValue(sporkDefT.sporkId));
            };
            for (const auto& sporkDefR : sporkDefsR) {
                ret.pushKV(sporkDefR.name, sporkManager.GetSporkValue(sporkDefR.sporkId));
            };
            for (const auto& sporkDefD : sporkDefsD) {
                ret.pushKV(sporkDefD.name, sporkManager.GetSporkValue(sporkDefD.sporkId));
            };
            return ret;
            ...
            for (const auto& sporkDefR : sporkDefsR) {
                ret.pushKV(sporkDefR.name, sporkManager.IsSporkActive(sporkDefR.sporkId));
            };
            for (const auto& sporkDefD : sporkDefsD) {
                ret.pushKV(sporkDefD.name, sporkManager.IsSporkActive(sporkDefD.sporkId));
            };
            return ret;
        }
    }
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"     (string, required) The axe address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The axe address validated\n"
            ...
            "}\n"
            "\nExamples:\n"
            + HelpExampleCli("validateaddress", "\"PXBjw3Bwm7tN1rJ83rBr49nhV1AwHToLXc\"")
            + HelpExampleRpc("validateaddress", "\"PXBjw3Bwm7tN1rJ83rBr49nhV1AwHToLXc\"")
        );
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The axe address to use for the signature.\n"
            ...
            "\nAs json rpc\n"
            + HelpExampleRpc("verifymessage", "\"PXBjw3Bwm7tN1rJ83rBr49nhV1AwHToLXc\", \"signature\", \"my message\"")
        );
...
UniValue getaddressmempool(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "]\n"
            "\nExamples:\n"
            + HelpExampleCli("getaddressmempool", "'{\"addresses\": [\"PXBjw3Bwm7tN1rJ83rBr49nhV1AwHToLXc\"]}'")
            + HelpExampleRpc("getaddressmempool", "{\"addresses\": [\"PXBjw3Bwm7tN1rJ83rBr49nhV1AwHToLXc\"]}")
        );
...
UniValue getaddressutxos(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "]\n"
            "\nExamples:\n"
            + HelpExampleCli("getaddressutxos", "'{\"addresses\": [\"PXBjw3Bwm7tN1rJ83rBr49nhV1AwHToLXc\"]}'")
            + HelpExampleRpc("getaddressutxos", "{\"addresses\": [\"PXBjw3Bwm7tN1rJ83rBr49nhV1AwHToLXc\"]}")
        );
...
UniValue getaddressdeltas(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1 || !request.params[0].isObject())
        throw std::runtime_error(
            ...
            "]\n"
            "\nExamples:\n"
            + HelpExampleCli("getaddressdeltas", "'{\"addresses\": [\"PXBjw3Bwm7tN1rJ83rBr49nhV1AwHToLXc\"]}'")
            + HelpExampleRpc("getaddressdeltas", "{\"addresses\": [\"PXBjw3Bwm7tN1rJ83rBr49nhV1AwHToLXc\"]}")
        );
...
UniValue getaddressbalance(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "}\n"
            "\nExamples:\n"
            + HelpExampleCli("getaddressbalance", "'{\"addresses\": [\"PXBjw3Bwm7tN1rJ83rBr49nhV1AwHToLXc\"]}'")
            + HelpExampleRpc("getaddressbalance", "{\"addresses\": [\"PXBjw3Bwm7tN1rJ83rBr49nhV1AwHToLXc\"]}")
        );
...
UniValue getaddresstxids(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "]\n"
            "\nExamples:\n"
            + HelpExampleCli("getaddresstxids", "'{\"addresses\": [\"PXBjw3Bwm7tN1rJ83rBr49nhV1AwHToLXc\"]}'")
            + HelpExampleRpc("getaddresstxids", "{\"addresses\": [\"PXBjw3Bwm7tN1rJ83rBr49nhV1AwHToLXc\"]}")
        );
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "axe-cli and the GUI. There is no server-side difference."
        );
...
static UniValue getinfo_deprecated(const JSONRPCRequest& request)
{
    throw JSONRPCError(RPC_METHOD_NOT_FOUND,
        "getinfo\n"
        "\nThis call was removed in version 1.6. Use the appropriate fields from:\n"
        ...
        "\naxe-cli has the option -getinfo to collect and format these in the old format."
    );
}
