...
inline int set_error(axeconsensus_error* ret, axeconsensus_error serror)
{
    if (ret)
        *ret = serror;
    return 0;
...
{
    return (flags & ~(axeconsensus_SCRIPT_FLAGS_VERIFY_ALL)) == 0;
}

int axeconsensus_verify_script(const unsigned char *scriptPubKey, unsigned int scriptPubKeyLen,
                                    const unsigned char *txTo        , unsigned int txToLen,
                                    unsigned int nIn, unsigned int flags, axeconsensus_error* err)
{
    if (!verify_flags(flags)) {
        return axeconsensus_ERR_INVALID_FLAGS;
    }
    try {
        TxInputStream stream(SER_NETWORK, PROTOCOL_VERSION, txTo, txToLen);
        CTransaction tx(deserialize, stream);
        if (nIn >= tx.vin.size())
            return set_error(err, axeconsensus_ERR_TX_INDEX);
...
unsigned int axeconsensus_version()
{
