    ...
    // after.
    {
        ...
        BOOST_CHECK_EQUAL(response.write(),
            strprintf("[{\"success\":false,\"error\":{\"code\":-1,\"message\":\"Rescan failed for key with creation "
                      ...
                      "by pruning or data corruption (see axed log for details) and could be dealt with by "
...
BOOST_FIXTURE_TEST_CASE(CreateTransactionTest, CreateTransactionTestSetup)
{
    ...
    // First run the tests with only one input containing 100k haks
    {
        coinControl.SetNull();
    ...
    // Now use 4 different inputs with a total of 100k hak
    {
        coinControl.SetNull();
    ...
    // Last use 10 equal inputs with a total of 100k hak
    {
        coinControl.SetNull();
