 ...
 * between axed, and axe-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    if (mode == HMM_BITCOIN_QT)
        strUsage += HelpMessageOpt("-windowtitle=<name>", _("Sets a window title which is appended to \"Axe Core - \""));
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if Axe Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if Axe Core is started while in a temporary directory.\n",
    ...
    if(fLiteMode) {
        InitWarning(_("You are starting in lite mode, most Axe-specific functionality is disabled."));
    }
