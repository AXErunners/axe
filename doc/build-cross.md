Cross-compiliation of Axe Core
===============================

Axe Core can be cross-compiled on Linux to all other supported host systems. This is done by changing
the `HOST` parameter when building the dependencies and then specifying another `--prefix` directory when building Axe.
...
Note that for WSL the Axe Core source path MUST be somewhere in the default mount file system, for
example /usr/src/axe, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
It's not possible to build the Axe Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the Axe Core source code).
