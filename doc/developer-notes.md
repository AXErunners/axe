...
Axe Core is a multithreaded application, and deadlocks or other multithreading bugs
...
([`valgrind.supp`](https://github.com/axerunners/axe/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_axe
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_axe --log_level=test_suite
$ valgrind -v --leak-check=full src/axed -printtoconsole
```
...
# A coverage report will now be accessible at `./test_axe.coverage/index.html`.
```
...
- ThreadSocketHandler : Sends/Receives data from peers on port 9937.
...
However, in open source software such as Axe Core, where everyone uses
...
pay attention to for reviewers of Axe Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Axe Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Axe Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `axe-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `axe-cli` and the GUI debug console use this table to determine how to
